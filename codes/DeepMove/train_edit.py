...
import json
from RnnNp import TrajPreSimple1
import time
import numpy as np
...
def generate_input_list(mode, run_idx, data, model_mode):
    ...
    # print(queue_len)
    users_acc = {}
    loc, tim, target, uid = list(), list(), list(), list()
    for c in range(queue_len):
        # optimizer.zero_grad()
        u, i = run_queue.popleft()
        if u not in users_acc:
...
def train_simple(model, loc, tim, y_train, lr, validation):
    ...
    loss, accuracy = model.calculate_loss(loc, tim, y_train)

    if not validation:
        for i in range(len(y_train)):
            # One SGD step
            model.sgd_step(loc[i], tim[i], y_train[i], lr)
        return model, loss
    # time = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
    # print("{0}: epoch={2}: loss={3: f} accuracy={4: .5f}%".format
    #       (time, loss, 100 * accuracy))
