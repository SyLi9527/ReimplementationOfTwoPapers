...
class TrajPreSimple(nn.Module):
    ...
    def __init__(self, parameters):
        ...
        # print(self.loc_size, self.loc_emb_size)
        ...
        # print(input_size, self.hidden_size)
        self.init_weights()
...
class Attn(nn.Module):
    ...
    def forward(self, out_state, history):
        ...
        score = F.softmax(attn_energies, dim=1)
        return score
...
class TrajPreAttnAvgLongUser(nn.Module):
    ...
    def __init__(self, parameters):
        ...
        self.fc_attn = nn.Linear(input_size, self.hidden_size).to('cpu')
    ...
    def forward(self, loc, tim, history_loc, history_tim, history_count, uid, target_len):
        ...
        count = 0
        loc_emb_history2 = Variable(torch.zeros(
            len(history_count), loc_emb_history.size()[-1]))
        tim_emb_history2 = Variable(torch.zeros(
            len(history_count), tim_emb_history.size()[-1]))
        ...
        history = torch.cat((loc_emb_history2, tim_emb_history2), 1).to('cuda')
    ...
    def forward(self, loc, tim, target_len):
        ...
        if self.rnn_type == 'GRU' or self.rnn_type == 'RNN':
            ...
            hidden_history, (h1, c1) = self.rnn_encoder(
                x[:-target_len], (h1, c1))
            hidden_state, (h2, c2) = self.rnn_decoder(
                x[-target_len:], (h2, c2))
