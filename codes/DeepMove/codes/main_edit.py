...
def run(args):
    ...
    if parameters.model_mode in ['simple', 'simple_long']:
        ...
        model = TrajPreSimple2(hidden_dim=parameters.hidden_size,
                               loc_dim=parameters.loc_size, loc_emb_size=parameters.loc_emb_size,
                               tim_emb_size=parameters.tim_emb_size)
    ...
    metrics = {'train_loss': [], 'valid_loss': [],
               ...
               'history_loc': [],
               'history_tim': [], 'history_count': [], 'uid': [], 'target_len': [],
               'test_history_tim': [], 'test_history_count': [], 'test_uid': [], 'test_target_len': []
               }
    ...
    if long_history is False:
        data_train, train_idx = generate_input_history(
            parameters.data_neural, 'train', mode2=parameters.history_mode,
            candidate=candidate)
        data_test, test_idx = generate_input_history(
            parameters.data_neural, 'test', mode2=parameters.history_mode,
            candidate=candidate)
        ...
        if parameters.model_mode == 'simple_long':
            ...
            data_test, test_idx = generate_input_long_history(
                parameters.data_neural, 'test', candidate=candidate)
            if parameters.model_mode == 'attn_avg_long_user':

                metrics['loc'], metrics['tim'], metrics['target'], metrics['history_loc'],
                metrics['history_tim'], metrics['history_count'], metrics['uid'],
                metrics['target_len'] = generate_input_list(
                    'train', train_idx, data_train, parameters.model_mode)

                metrics['test_loc'], metrics['test_tim'], metrics['test_target'], metrics['test_history_loc'],
                metrics['test_history_tim'], metrics['test_history_count'], metrics['test_uid'],
                metrics['test_target_len'] = generate_input_list(
                    'test', test_idx, data_test, parameters.model_mode)
            else:

                metrics['loc'], metrics['tim'], metrics['target'],
                metrics['target_len'] = generate_input_list(
                    'train', train_idx, data_train, parameters.model_mode)

                metrics['test_loc'], metrics['test_tim'], metrics['test_target'],
                metrics['test_target_len'] = generate_input_list(
                    'test', test_idx, data_test, parameters.model_mode)
        ...
        if args.pretrain == 1:
            ...
            if parameters.model_mode in ['simple', 'simple_long']:
                ...
                model, avg_loss = train_simple(
                    model, loc_list, tim_list, target, lr, False)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--voc_emb_size', type=int,
                        default=30, help="words embeddings size")
    parser.add_argument('--tim_emb_size', type=int,
                        default=5, help="time embeddings size")
    parser.add_argument('--hidden_size', type=int, default=35)
