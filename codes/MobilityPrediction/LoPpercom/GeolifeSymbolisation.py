...
main_geolifedb = '../DataGeolife/geolife.sqlite'
# main_geolifedb = '../DataGeolife/geolifepart.sqlite'
# folder for semi-temporary databases for specific quantisations, will be created if they don't exist
preprocessing_dir = '../DataGeolife/Preproc'
# preprocessing_dir = '../DataGeolife/Preprocpart'
# folder for the original data. Used to build the above databases if they don't exist.
geolife_zipfile_data_dir = '../Geolife Trajectories 1.3/Data'
# geolife_zipfile_data_dir = '../Geolife Trajectories 1.3part/Data'
...
def build_specific_cache(spatialRes_temporalRes_pair):
    ...
    print('Processing spatial res {}, temporal res {}'.format(
...
def bulk_build_resolution_cache(listSpatialRes, listTemporalRes, personsId="All"):
    ...
    pool.map(build_specific_cache, pairs)
    pool.close()
    pool.join()
...
def get_geolife_data(spatialRes, temporalRes, personsId="All"):
    ...
    if not os.path.exists("{}/S{}T{}.sqlite".format(preprocessing_dir, spatialRes, temporalRes)):
        ...
        buildPreprocessingTable(spatialRes, temporalRes,
                                nest=True, personsIds=personsId)
...
def loadData(spatialRes, temporalRes, personsId="All", withDates=False):
    ...
    connection = apsw.Connection(
        "{}/S{}T{}.sqlite".format(preprocessing_dir, spatialRes, temporalRes))
...
#        idPerson.append(person)
        ...
        sql = "SELECT DISTINCT traj FROM preproc WHERE person = {} AND NOT datetime = '' GROUP BY traj".format(
            person)
        ...
        for t in trajectories:
            ...
            sql = "SELECT idxPix, datetime FROM preproc WHERE person = {} AND traj = {} ORDER BY datetime".format(
                person, traj)
...
def ComputeNside(spatialRes):
    ...
    print("nearestSpatialRes = {}, Npix = {} and Nside = {}".format(
        nearestSpatialRes, npix, nside))
    return nside
...
#     writingCurs.execute(sql)
    writingCurs.execute(
        "CREATE TABLE preproc (person INT, traj INT, idxPix INT, datetime TEXT)")
...
#
    for pId in personsIds:
        ...
        print("Considering s: {} t:{} Person {}".format(
            spatialRes, temporalRes, pId))

        person = pId
#        person =0
        sql = "SELECT distinct traj FROM geolife WHERE person = {} AND NOT datetime = '' GROUP BY traj".format(
            person)
    #    AND traj = 20090426211055
        ...
        for t in trajectories:
            ...
            sql = "SELECT longitude ,latitude , datetime FROM geolife WHERE person = {} AND NOT datetime = '' AND traj = {} ORDER BY datetime".format(
                person, traj)
            ...
            for row in rows:
                ...
                if ct == 0:
                    ...
                    if t_ct == 0:
                        ...
                        nextTime = actualTime
                        points.append(
                            (person, traj, int(getIdxPix(row[0], row[1])), nextTime.strftime(fmt)))
                        nextTime += temporalRes

                    else:
                        # Determine the number of symbols missing.
                        nb_loc_missing = int(
                            (actualTime - nextTime).total_seconds()//temporalRes.total_seconds()+1)
    ...
    #                        print "Fill blank-----------------------------------------------------", nb_loc_missing
                # Else if the time is over the next time, this mean that we have to record the point
                    ...
                    # Or the previous one ?
                        ...
                        # record the point :
                        points.append(
                            (person, traj, int(getIdxPix(prev[0], prev[1])), nextTime.strftime(fmt)))
                    nextTime += temporalRes
            ...
            if len(points) > 0:
                data.append(points)
                if len(points) > 1:
                    writingCurs.executemany(
                        "INSERT INTO preproc (person,traj,idxPix,datetime) VALUES (?,?,?,?)", points)

            print("S: {} T: {} Person {}: {} trajectories processed".format(
                spatialRes, temporalRes, person, t_ct))
            ...
            with open(mainDirectory + '/' + person + '/Trajectory/' + trajectoryFile, 'r') as f:
                ...
                for line in f:
                    if ct >= 6:
                        data = line.split(',')
                        sqlList.append(
                            (person, id_trajectory, data[0], data[1], data[-2] + " " + data[-1].replace('\n', '').replace('\r', '')))

                    ct += 1

            f.close()

        # turn of journalling for speed
        curs1orig.execute('PRAGMA journal_mode = OFF; ')
        # this will disable autocommit for speed, bundling it into a single commit
        curs1orig.execute('BEGIN')
